...
from drs_downloader.models import AccessMethod
...
def terra(silent: bool, destination_dir: str, manifest_path: str, drs_header: str):
    ...
    def _extract_tsv_info(manifest_path_: Path, drs_header: str) -> List[str]:
        ...
        uri_index = 0
        header = None
        with open(Path(manifest_path_)) as file:
            ...
            if (header is not None):
                ...
                raise KeyError("Key format for drs_uri is bad. Make sure the collumn that contains the URIS \
                has 'uri' somewhere in it, or the URI header matches the uri header name in the TSV file that was specified")
            
            for url in urls:
                if('/' in url):continue
                else:
                    raise Exception("Check that your header name for your DRS URIS is directly above the column of your DRS URIS")
            return urls
    ...
    #size_list = [objects.size for objects in drs_objects]
    ...
    for chunk_of_object_ids in tqdm.tqdm(
        ...
        # for some reason the objectgs don't stay sorted so you need to sort them here to make sure that the right files are matching up
