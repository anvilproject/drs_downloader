...
import json
import os.path
from drs_downloader.DRSClient import DRSClient
...
class bdcDRSClient(Gen3DRSClient):
    ...
    def __init__(self, api_key_path, access_id=None, debug=False):
        super().__init__('https://development.aced-idp.org', '/user/credentials/cdis/access_token',
                         api_key_path, access_id, debug)
...
class anvilDRSClient(Gen3DRSClient):
    ...
    def __init__(self, api_key_path, userProject=None, access_id=None, debug=False):
        ...
        super().__init__('https://gen3.theanvil.io', '/user/credentials/api/access_token',
                         api_key_path, access_id, debug)
...
if __name__ == "__main__":
    ...
    test_data = {'BioDataCatalyst': {'drs_client': bdcDRSClient('~/Desktop/credentials.json', 's3', debug=True),
                                     'drs_ids': ['f0027bc1-7c62-4fa4-9f8f-5e6f396c6333']}}

    for (testname, test) in test_data.items():
        print('______________________________________')
        print(testname)
        drsClient = test['drs_client']
        if drsClient.api_key is None:
            print("This DRS client failed to obtain authorization. Check credentials file")
        ...
        for drs_id in test['drs_ids']:
            ...
            try:
                ...
                if drsClient.api_key is None:
                    print("This DRS client has not obtained authorization and cannot obtain URLs for "
                          "controlled access objects")
                else:
