...
def test_extract_manifest_info():
    manifest_path = "tests/fixtures/manifests/terra-data.tsv"
    ...
    with pytest.raises(KeyError):
        _extract_manifest_info("tests/fixtures/manifests/no-header.tsv", None)

    # No header found in manifest file despite user passing in header value
    with pytest.raises(KeyError):
        _extract_manifest_info("tests/fixtures/manifests/no-header.tsv", drs_header)
